...
/*
定一个Person类 {name,age,job} ,初始化Person 对象数组，有3个person对象，
并按照 age 从大到小 进行排序，提示 ，使用冒泡
 */
...
public class Homework01 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < persons.length; i++) {
            System.out.println(persons[i]); // 默认调对象的 .toString()方法
        ...
        for (int i = 0; i < persons.length - 1; i++) { //外层循环
            for (int j = 0; j < persons.length - 1 - i; j++) { //内层循环
                //按照名字长度从小到大进行排序，如果前面的人名字长度 > 后面人的年龄，就交换
               // if(persons[i].getName().length() > persons[i+1].getName().length()) {

                //按照age从大到小进行排序，如果前面的人 age < 后面人的年龄，就交换
        ...
        System.out.println("age从大到小排序后的效果");
        for (int i = 0; i < persons.length; i++) {
            System.out.println(persons[i]); // 默认调对象的 .toString()方法
